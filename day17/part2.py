...
DEFAULT_INPUT_FILE = "input/part2.txt"
...
class Direction(Enum):
	...
	def right_turn_coords(self):
		i = self.get_turn_index('R')
		return XY_OFFSETS[i]

	def left_turn_coords(self):
		i = self.get_turn_index('L')
		return XY_OFFSETS[i]

	def get_turn_index(self, which):
		if which == 'R':
			if self.value == 0:
				i = 3
			elif self.value == 1:
			...
			raise Exception("Unexpected turn value: {}".format(which))
		return i

	def turn(self, which):
		i = self.get_turn_index(which)
		return Direction(i)
...
def is_intersection(view, x, y):
	try:
		if [view[y][x], view[y-1][x], view[y+1][x], view[y][x-1], view[y][x+1]].count('#') == 5:
			return True
	except Exception:
		pass
	return False
...
def get_intersections(view):
	intersections = []
	for y in xrange(1, len(view) - 1):
		for x in xrange(1, len(view[0]) - 1):
			if is_intersection(view, x, y):
				intersections.append((x,y))
	return intersections

def get_next_xy(d, x, y):
	offset = XY_OFFSETS[d.value]
	return (x + offset[0], y + offset[1])

def get_turn(view, d, x, y):
	r_x, r_y = d.right_turn_coords()
	l_x, l_y = d.left_turn_coords()

	if view[r_y + y][r_x + x] == SCAFFOLD:
		return 'R'
	elif view[l_y + y][l_x + x] == SCAFFOLD:
		return 'L'
	return None

def in_bounds(view, x, y):
	if x >= 0 and x < len(view[0]) and y >= 0 and y < len(view) and x < len(view[y]):
		return True
	return False

def get_path_instructions(view, bot_xy):
	x,y = bot_xy
	c = view[y][x]
	...
	curr_y = y
	while True:
		# Try to go forward as far as you can
		n = 0
		while True:
			next_x, next_y = get_next_xy(direction, curr_x, curr_y)
			if in_bounds(view, next_x, next_y) and view[next_y][next_x] == SCAFFOLD:
				n += 1
				curr_x = next_x
				curr_y = next_y
			else:
		...
		if n:
			ins.append(n)
		turn = get_turn(view, direction, curr_x, curr_y)
		if not turn:
			# All done!
			return ins
		ins.append(turn)
...
def main(args):
	...
	# halted = computer.go()
	# print("Halted: {}".format(halted))

	# view = []
	# o = computer.get_output()
	# row = []
	# while o:
	# 	if o == 10:
	# 		view.append(row)
	# 		row = []
	# 	elif o == 60 or o == 62 or o == 118 or o == 94:
	# 		bot_location = (len(row), len(view))
	# 		row.append(chr(o))
	# 	else:
	# 		row.append(chr(o))
	...
	# print("bot location: {},{}".format(bot_location[0], bot_location[1]))

	# ins = get_path_instructions(view, bot_location)

	# print(",".join(map(str, ins)))


	# computer.set_instruction_value(0, 0, 2)
	...
	print(computer.io_in)

	halted = computer.go()
	if not halted:
		raise Exception("Still looking for input, but I have none to give")

	print(computer.io_out)
